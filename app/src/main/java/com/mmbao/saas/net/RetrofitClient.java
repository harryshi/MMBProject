...
import com.mmbao.saas.util.ExceptionHandle;
 ...
 * 参考网址：http://blog.csdn.net/sk719887916/article/details/51958010
 */
public class RetrofitClient {
    ...
    public static String baseUrl = ApplicationGlobal._appUrl;
    ...
    //如果需要url的话可以在创建一个静态初始化器，带2个参数，并释放下面方法
    ...
    private RetrofitClient(Context context, String url) {
        ...
        try {
            ...
            headers.put("token", SystemInfo.getInstance(context).getToken());
            headers.put("User-Agent", Cookies.User_Agent);
            headers.put("imei", MMBApplication.getInstance().imei);
            headers.put("memberId", SystemInfo.getInstance(context).getMemberid());
        }catch (Exception e){
            Log.e("headers", "Could not create headers", e);
        }
...
//                .connectionPool(new ConnectionPool(8, 15, TimeUnit.SECONDS))
   ...
   /**
    ...
    public static void changeApiHeader(Map<String, String> newApiHeaders) {
        ...
        okHttpClient.newBuilder().addInterceptor(new BaseInterceptor(mContext, newApiHeaders)).build();
     ...
     */
    public  <T> T create(final Class<T> service) {//网络层调用接口时用这个
        if (service == null) {
    ...
    //rxJava的转换器  解决上面每个api都得进行指定线程的繁琐
    ...
    public <T> Observable.Transformer<BaseBean<T>, T> transformer() {
    ...
    private class HandleFuc<T> implements Func1<BaseBean<T>, T> {
        @Override
        public T call(BaseBean<T> response) {
